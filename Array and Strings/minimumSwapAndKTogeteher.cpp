...
// Medium Accuracy: 47.98% Submissions: 59446 Points: 4
...
// Min no swap to bring  all elements less than k at start of the array

// COUNT MORE THAN K ELEMENTS IN EVERY WINDOW
...
int minSwap(int arr[], int n, int k)
{
    ...
    for (int i = window; i < n; i++)
    {
        ...
        if (arr[i - window] > k)
            morethanK--;
        minSwaps = min(morethanK, minSwaps);
    }
    return minSwaps;
}